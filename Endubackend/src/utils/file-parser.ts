...
export interface CourseData {
  ...
  enrollment_policy: 'open' | 'invite_only'
  modules: ModuleData[]
}
...
// Parse Excel file
export async function parseExcelFile(buffer: Buffer): Promise<CourseData[]> {
  try {
...
// Parse CSV file
export async function parseCSVFile(buffer: Buffer): Promise<CourseData[]> {
...
function convertCSVToCourseData(rows: any[]): CourseData[] {
  const courseGroups = new Map<string, any[]>()
  
  for (const row of rows) {
    if (!row.course_title) continue
...
// Convert CSV rows to CourseData
function convertCSVRowsToCourseData(courseTitle: string, rows: any[]): CourseData {
  const firstRow = rows[0]
  
  const course: CourseData = {
    title: courseTitle,
    description: firstRow.course_description || '',
    status: (firstRow.course_status as 'draft' | 'published') || 'draft',
    visibility: (firstRow.course_visibility as 'private' | 'public') || 'private',
    enrollment_policy: (firstRow.enrollment_policy as 'open' | 'invite_only') || 'invite_only',
    modules: []
  }
  ...
  for (const [moduleTitle, moduleRows] of moduleGroups) {
    ...
    for (const row of moduleRows) {
      ...
      const lesson: LessonData = {
        ...
        type: (row.lesson_type as 'video' | 'text' | 'quiz' | 'assignment') || 'text',
      ...
      if (row.quiz_question && row.quiz_question.trim()) {
        const quizQuestion: QuizQuestionData = {
          question: row.quiz_question,
          type: (row.quiz_type as 'multiple-choice' | 'true-false' | 'fill-blank') || 'multiple-choice',
...
function convertRowsToCourseData(courseTitle: string, rows: any[][], headers: string[]): CourseData {
  const firstRow = rows[0]
  
  const course: CourseData = {
    title: courseTitle,
    description: getValueByHeader(firstRow, headers, 'course_description') || '',
    status: (getValueByHeader(firstRow, headers, 'course_status') as 'draft' | 'published') || 'draft',
    visibility: (getValueByHeader(firstRow, headers, 'course_visibility') as 'private' | 'public') || 'private',
    enrollment_policy: (getValueByHeader(firstRow, headers, 'enrollment_policy') as 'open' | 'invite_only') || 'invite_only',
    modules: []
  }
  ...
  for (const [moduleTitle, moduleRows] of moduleGroups) {
    ...
    for (const row of moduleRows) {
      ...
      const lesson: LessonData = {
        ...
        type: (getValueByHeader(row, headers, 'lesson_type') as 'video' | 'text' | 'quiz' | 'assignment') || 'text',
      ...
      if (quizQuestion && quizQuestion.trim()) {
        const quizQuestionData: QuizQuestionData = {
          question: quizQuestion,
          type: (getValueByHeader(row, headers, 'quiz_type') as 'multiple-choice' | 'true-false' | 'fill-blank') || 'multiple-choice',
...
function getValueByHeader(row: any[], headers: string[], headerName: string): string {
...
export function validateCourseData(courses: CourseData[]): { isValid: boolean; errors: string[] } {
  const errors: string[] = []
