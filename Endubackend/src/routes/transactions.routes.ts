...
router.get('/teacher/:email/summary', requireAuth, async (req: Request, res: Response) => {
  try {
    const { email } = req.params;
...
router.patch('/:transaction_id/status', async (req: Request, res: Response) => {
  try {
