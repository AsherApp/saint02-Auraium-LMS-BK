...
router.post('/', requireAuth, async (req, res) => {
    try {
        ...
        const { course_id, title, description, instructions, type, scope, points, due_at, available_from, available_until, allow_late_submissions, late_penalty_percent, max_attempts, time_limit_minutes, require_rubric, rubric, resources, settings } = req.body;
...
router.get('/:id/grading-stats', requireAuth, async (req, res) => {
    try {
        ...
        const stats = {
            ...
            completionRate: totalSubmissions > 0 ? (submittedCount / totalSubmissions) * 100 : 0,
            gradingProgress: totalSubmissions > 0 ? (gradedCount / totalSubmissions) * 100 : 0
        };
        res.json(stats);
    }
    catch (error) {
