...
router.get('/', requireAuth, asyncHandler(async (req, res) => {
    ...
    const uniqueStudents = (data || []).filter((student, index, self) => index === self.findIndex((s) => s.email === student.email));
...
router.get('/with-enrollments', requireAuth, asyncHandler(async (req, res) => {
    ...
    const studentsWithEnrollments = (data || []).map((student) => {
...
router.get('/consolidated', requireAuth, asyncHandler(async (req, res) => {
    ...
    const consolidatedStudents = await Promise.all((data || []).map(async (student) => {
        ...
        const activeCourses = enrollments.filter((e) => e.courses?.status === 'draft' || e.courses?.status === 'published').length;
...
router.get('/:email/course/:courseId/details', requireAuth, asyncHandler(async (req, res) => {
    ...
    const { data: grades, error: gradesError } = await supabaseAdmin
        ...
        .order('created_at', { ascending: false });
    if (gradesError)
        console.error('Error getting grades:', gradesError);
    // Get real activities
    ...
    const assignmentGrades = (grades || []).filter((g) => g.assignment_id);
    const quizGrades = (grades || []).filter((g) => g.grade_type === 'quiz');
    const averageAssignmentScore = assignmentGrades.length > 0
        ? Math.round(assignmentGrades.reduce((sum, g) => sum + g.grade_percentage, 0) / assignmentGrades.length)
        : null;
    const averageQuizScore = quizGrades.length > 0
        ? Math.round(quizGrades.reduce((sum, g) => sum + g.grade_percentage, 0) / quizGrades.length)
        : null;
    const overallGrade = (grades || []).length > 0
        ? Math.round((grades || []).reduce((sum, g) => sum + g.grade_percentage, 0) / (grades || []).length)
        : null;
    const courseDetails = {
        // Basic info
        student: {
        ...
        progress: {
            overall_percentage: progressData.progress_percentage || enrollment.progress_percentage || 0,
            modules_completed: progressData.completed_lessons || 0,
            total_modules: progressData.total_lessons || 0,
            lessons_completed: progressData.completed_lessons || 0,
            total_lessons: progressData.total_lessons || 0,
            time_spent: progressData.total_time_spent_hours || 0, // hours
            last_activity: progressData.last_activity || enrollment.last_activity
        },
...
router.get('/:email/poll-participation', requireAuth, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const pollData = pollActivities.map((activity) => ({
            id: activity.id,
...
router.get('/:email/discussion-participation', requireAuth, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (error)
            throw error;
        // Transform data for frontend
        const discussionData = discussionActivities.map((activity) => ({
            id: activity.id,
...
router.get('/:email/study-time', requireAuth, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (error)
            throw error;
        const totalSeconds = progressData.reduce((sum, progress) => sum + (progress.time_spent_seconds || 0), 0);
        res.json({
