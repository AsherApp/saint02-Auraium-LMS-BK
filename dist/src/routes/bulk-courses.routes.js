...
import { parseExcelFile, parseCSVFile, generateExcelTemplate, generateCSVTemplate, validateCourseData } from '../utils/file-parser.js';
...
router.post('/upload', requireAuth, upload.single('file'), asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const bulkCourses = courses.map(course => ({
            title: course.title,
            ...
            modules: course.modules.map(module => ({
                ...
                lessons: module.lessons.map(lesson => ({
                    ...
                    content: {
                        ...
                        video: lesson.type === 'video' ? { url: lesson.content, source: 'upload' } : undefined,
        ...
        res.json({
            ...
            summary: {
                ...
                totalLessons: bulkCourses.reduce((sum, course) => sum + course.modules.reduce((moduleSum, module) => moduleSum + module.lessons.length, 0), 0)
            }
        });
    }
    catch (error) {
