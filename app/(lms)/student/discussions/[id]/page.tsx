...
export default function StudentDiscussionDetailPage() {
  ...
  const fetchDiscussion = async () => {
    try {
      ...
      // Only show approved posts for students
      setPosts((response.posts || []).filter(post => post.is_approved))
    } catch (error: any) {
  ...
  const handlePostReply = async (parentPostId?: string) => {
    ...
    if (!discussion?.allow_student_posts) {
      toast({
        title: "Error",
        description: "Students are not allowed to post in this discussion",
        variant: "destructive"
      })
      return
    }
    ...
    try {
      ...
      toast({
        title: "Success",
        description: discussion.require_approval 
          ? "Reply posted and is pending approval" 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {getMainPosts().map((post) => (
          <div key={post.id}>
            ...
            {/* Reply Form */}
            {replyingTo === post.id && discussion.allow_student_posts && (
      ...
      {/* New Post Form */}
      {!discussion.is_locked && discussion.allow_student_posts && (
        <GlassCard className="p-6">
          <h3 className="text-lg font-semibold text-white mb-4">Add a Post</h3>
          {discussion.require_approval && (
            <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
              <p className="text-yellow-400 text-sm">
                Note: Your posts will need to be approved by the teacher before they appear.
              </p>
            </div>
          )}
          <Textarea
      ...
      {!discussion.allow_student_posts && (
        <GlassCard className="p-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">Students Cannot Post</h3>
          <p className="text-slate-400">This discussion is read-only for students.</p>
        </GlassCard>
      )}
    </div>
  )
